...
import type { ComponentType, FC, FunctionComponent, JSXElementConstructor, ReactElement, ReactNode } from 'react';
...
import type { SORT_DIRECTION_ASCENDING, SORT_DIRECTION_DESCENDING, SORT_DIRECTION_NONE } from './constants';
...
import type { I18N_FIELD_DUPLICATE, I18N_FIELD_NONE, I18N_FIELD_TRANSLATE, I18N_STRUCTURE_MULTIPLE_FILES, I18N_STRUCTURE_MULTIPLE_FOLDERS, I18N_STRUCTURE_SINGLE_FILE } from './lib/i18n';
...
export type Group = Record<string, Record<string, GroupMap>>;
...
export type ValueOrNestedValue = string | number | (string | number)[] | boolean | ObjectValue | Date | ValueOrNestedValue[] | null | undefined;
...
export type FieldGetValidValueMethod<T = unknown, F extends BaseField = UnknownField> = (value: T | undefined | null, field: F) => T | undefined | null;
export type FieldGetDefaultMethod<T = unknown, F extends BaseField = UnknownField> = (defaultValue: T | undefined | null, field: F) => T;
export interface FieldValidationMethodProps<T = unknown, F extends BaseField = UnknownField> {
    field: F;
    value: T | undefined | null;
    t: t;
}
export type FieldValidationMethod<T = unknown, F extends BaseField = UnknownField> = (props: FieldValidationMethodProps<T, F>) => false | FieldError | Promise<false | FieldError>;
export interface FieldStorageConverters<T = unknown, F extends BaseField = UnknownField, S = ValueOrNestedValue> {
...
export interface CollectionFileWithDefaults<EF extends BaseField = UnknownField> extends Omit<CollectionFile<EF>, 'i18n'> {
    i18n?: I18nInfo;
}
interface Nested {
    summary?: string;
...
export interface FilesCollectionWithDefaults<EF extends BaseField = UnknownField> extends BaseCollectionWithDefaults {
...
export type FolderCollection<EF extends BaseField = UnknownField> = BaseCollection & BaseFolderCollection<EF>;
export type FolderCollectionWithDefaults<EF extends BaseField = UnknownField> = BaseCollectionWithDefaults & BaseFolderCollection<EF>;
export type Collection<EF extends BaseField = UnknownField> = FilesCollection<EF> | FolderCollection<EF>;
export type CollectionWithDefaults<EF extends BaseField = UnknownField> = FilesCollectionWithDefaults<EF> | FolderCollectionWithDefaults<EF>;
export type Collections<EF extends BaseField = UnknownField> = Record<string, Collection<EF>>;
export type CollectionsWithDefaults<EF extends BaseField = UnknownField> = Record<string, CollectionWithDefaults<EF>>;
export type MediaFile = BackendMediaFile & {
    key?: string;
};
...
export type WidgetPreviewComponent<T = unknown, F extends BaseField = UnknownField> = ReactElement<unknown, string | JSXElementConstructor<any>> | ComponentType<WidgetPreviewProps<T, F>>;
export type WidgetFor<P = ObjectValue> = <K extends keyof P>(name: K) => ReactNode;
export type WidgetsFor<P = ObjectValue> = <K extends keyof P>(name: K) => P[K] extends Array<infer U> ? {
    data: U | null;
...
export type TemplatePreviewComponent<T = ObjectValue, EF extends BaseField = UnknownField> = ComponentType<TemplatePreviewProps<T, EF>>;
...
export type TemplatePreviewCardComponent<T = EntryData, EF extends BaseField = UnknownField> = ComponentType<TemplatePreviewCardProps<T, EF>>;
export interface FieldPreviewProps<T = unknown, F extends BaseField = UnknownField> {
    collection: CollectionWithDefaults<F>;
    field: Field<F>;
    value: T;
}
export type FieldPreviewComponent<T = unknown, F extends BaseField = UnknownField> = ComponentType<FieldPreviewProps<T, F>>;
export interface WidgetOptions<T = unknown, F extends BaseField = UnknownField, S = ValueOrNestedValue> {
...
export declare abstract class BackendClass {
    ...
    abstract entriesByFolder(folder: string, extension: string, depth: number): Promise<ImplementationEntry[]>;
    ...
    abstract getMedia(mediaFolder?: string, folderSupport?: boolean, publicFolder?: string): Promise<ImplementationMediaFile[]>;
    ...
    abstract allEntriesByFolder(folder: string, extension: string, depth: number, pathRegex?: RegExp): Promise<ImplementationEntry[]>;
    abstract traverseCursor(cursor: Cursor, action: string): Promise<{
    ...
    abstract unpublishedEntryDataFile: (collection: string, slug: string, path: string, id: string) => Promise<string>;
    abstract unpublishedEntryMediaFile: (collection: string, slug: string, path: string, id: string) => Promise<ImplementationMediaFile>;
    abstract updateUnpublishedEntryStatus: (collection: string, slug: string, newStatus: WorkflowStatus) => Promise<void>;
    ...
    abstract getDeployPreview: (collectionName: string, slug: string) => Promise<{
        url: string;
        status: string;
    } | null>;
}
...
export type Field<EF extends BaseField = UnknownField> = BooleanField | CodeField | ColorField | DateTimeField | FileOrImageField | KeyValueField | ListField<EF> | MapField | MarkdownField | NumberField | ObjectField<EF> | RelationField | SelectField | HiddenField | StringField | TextField | UUIDField | EF;
...
export interface ConfigWithDefaults<EF extends BaseField = UnknownField> extends Omit<Config<EF>, 'collections'> {
...
export type EventListener = PrePublishEventListener | PostPublishEventListener | PreSaveEventListener | PostSaveEventListener | ChangeEventListener | LoginEventListener | LogoutEventListener | MountedEventListener;
...
export type I18nStructure = typeof I18N_STRUCTURE_MULTIPLE_FILES | typeof I18N_STRUCTURE_MULTIPLE_FOLDERS | typeof I18N_STRUCTURE_SINGLE_FILE;
export type I18nField = typeof I18N_FIELD_DUPLICATE | typeof I18N_FIELD_TRANSLATE | typeof I18N_FIELD_NONE;
...
export type MarkdownToolbarItemsFactory = (props: MarkdownToolbarItemsFactoryProps) => (string | any)[][];
