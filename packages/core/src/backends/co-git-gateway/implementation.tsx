...
export default class GitGateway implements BackendClass {
  ...
  requestFunction = (req: ApiRequest) =>
    this.tokenPromise!()
      .then(
        token => unsentRequest.withHeaders({ Authorization: `Bearer ${token}`, 'x-mf-co-project': `_${window.location.pathname.split("/")[1]}` }, req) as ApiRequest,
      )
      .then(unsentRequest.performRequest);

  authenticate() {
    this.tokenPromise = async () => {
      return "token_lol" // get authentication header     return token;
    }
    return this.tokenPromise!().then(async token => {
      if (!this.backendType) {
        const {
          ...
          .fetchWithTimeout(`${this.gatewayUrl}/settings`, {
            headers: { Authorization: `Bearer ${token}`, 'x-mf-co-project': `_${window.location.pathname.split("/")[1]}` },
          })
          .then(async res => {
      ...
      const apiConfig = {
        ...
        commitAuthor: { name: "user", email: "couser@moonface.digital" },
      ...
      return { name: "user", login: "login" } as User;
    });
  }
